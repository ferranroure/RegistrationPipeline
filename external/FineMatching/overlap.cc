...
//cout<<"overlap:cc find ovelaproneduit aua2"<<endl;

	for (int i = 0; i < nsamp; i++) {
		int ind = int((float) i / nsamp * nv);
		ind = clamp(ind, 0, nv-1);
//cout<<"overlap:cc find ovelaproneduit aua25"<<endl;
...
//cout<<"overlap:cc find ovelaproneduit aua27"<<endl;

		if (!q)
			continue;

//cout<<"overlap:cc find ovelaproneduit aua270"<<endl;

		int ind2 = (q - (const float *) &(mesh2->vertices[0][0])) / 3;
//cout<<"overlap:cc find ovelaproneduit aua271 "<<ind2<<" "<<mesh2->vertices[0]<<" "<<*q<<endl;
		if (mesh2->is_bdy(ind2))
			continue;
//cout<<"overlap:cc find ovelaproneduit aua272"<<endl;

		if (((xf12r * mesh1->normals[ind]) DOT mesh2->normals[ind2])
				<= 0.0f)
			continue;
//cout<<"overlap:cc find ovelaproneduit aua273"<<endl;
		area += this_area;
		rmsdist += this_area *
			sqr((p - point(q)) DOT mesh2->normals[ind2]);
//cout<<"overlap:cc find ovelaproneduit aua29"<<endl;

	}
//cout<<"overlap:cc find ovelaproneduit aua3"<<endl;
