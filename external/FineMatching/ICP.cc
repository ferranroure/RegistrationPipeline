							 ...
							 vector<PtPair> &pairs, bool flip)
{
	...
	while (1) {
		...
		cval += incr * tinyrnd();
		//cout << cval << " = " << incr << " * " << tinyrnd() << endl;
		if (cval >= 1.0f)
			break;
		while (sampcdf1[i] <= cval)
			i++;
		cval = sampcdf1[i];
		//cout << "cval modified: " << cval << " i: " << i << " size: " << sampcdf1.size() << endl;
		//cin.ignore(1);
...
static void compute_ICPmatrix(const vector<PtPair> &pairs, float evec[6][6], float eval[6],
							  float b[6], point &centroid, float &scale, float &err)
{
...
//cout << "values: " << sampcdf1[0] << " " << sampcdf2[0] << endl;
	...
	point centroid;
	xform alignxf;
	compute_ICPmatrix(pairs, evec, eval, b, centroid, scale,
					  err);
	if (verbose > 1) {
		  ...
		  bool do_scale /* = false */, bool do_affine /* = false */)
{
	...
	do {
		...
		// SAVING POINTS INTO FILE --------------------------------------
//		cout << "ICP ERROR: " << err <<  endl;
//
//		stringstream ss;
//		ss << err;
//		string file = "../models/icpresults/file"+ss.str()+".ply";
//		ofstream outfile (file.c_str());
//		// Printing the header
//		outfile << "ply" << endl;
//		outfile << "format ascii 1.0 " << endl;
//		outfile << "element vertex " << s2->vertices.size() << endl;
